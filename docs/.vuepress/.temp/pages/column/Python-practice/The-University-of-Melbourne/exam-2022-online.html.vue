<template><div><h2 id="question1" tabindex="-1"><a class="header-anchor" href="#question1" aria-hidden="true">#</a> Question1</h2>
<p>Construct a <strong>single</strong> Python expression which evaluates to the following values, and incorporates the specified operations in each case (executed in any order).</p>
<blockquote>
<p>构造一个 Python 表达式，求值为以下值，并在每种情况下合并指定的操作(以任何顺序执行)。</p>
</blockquote>
<p><strong>(a) Output value: 'grin'</strong></p>
...
<p><strong>(b) Output value: True</strong></p>
<p>Required operations:</p>
<ol>
<li>
<p>range() function</p>
...
<p><strong>(c) Output value: 2</strong></p>
<p>Required operations:</p>
<ol>
<li>
<p>(float) division</p>
...
<p><strong>(d) Output value: 0</strong></p>
<p>Required operations:</p>
<ol>
...
<p><strong>(e) Output value:  'ace'</strong></p>
<p>Required operations:</p>
<ol>
<li>
<p>items method</p>
...
<blockquote>
<p>下面的函数旨在计算两个名称之间的相似度，方法是计算第一个名称的“n-grams”(n个字符的子字符串)在第二个名称中所占的比例，并对n从2到maxn(默认为3)的每个值的结果值进行平均。如果第一个名称的字符数少于n的任何测试值，函数应该返回0.0。</p>
</blockquote>
<p>Example function calls are:</p>
...
<span class="token number">0.0</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>All the required lines of the function are available, out of order, in the editor panel. Drag each line across to the solution area to the right, put them in the correct order, and introduce appropriate indentation. Note that ALL lines must be used in your solution.</p>
<blockquote>
<p>在编辑器面板中，函数所需的所有行都是可用的，但顺序不同。将每一行拖到解决方案区域的右侧，将它们按正确的顺序排列，并引入适当的缩进。注意，解决方案中必须使用ALL行。</p>
</blockquote>
<p><img src="@source/column/Python-practice/The-University-of-Melbourne/exam-2022-online.assets/image-20221031161310234.png" alt="image-20221031161310234" loading="lazy"></p>
<p><img src="@source/column/Python-practice/The-University-of-Melbourne/exam-2022-online.assets/image-20221031161256042.png" alt="image-20221031161256042" loading="lazy"></p>
<h2 id="question3" tabindex="-1"><a class="header-anchor" href="#question3" aria-hidden="true">#</a> Question3</h2>
<p>变成 while loop</p>
<div class="language-python line-numbers-mode" data-ext="py"><pre v-pre class="language-python"><code>SUIT <span class="token operator">=</span> <span class="token number">1</span>
...
<p>Sandy has enrolled in a Bachelor of Magical Arts. In order to graduate, they must complete the subject WIZ90001 Advanced Wizardry. However, this subject has a number of prerequisites, each of which must be completed before they can take WIZ90001. Each of those subjects in turn may have its own prerequisites, and so on.</p>
<blockquote>
<p>桑迪已经注册了魔法艺术学士学位。为了毕业，他们必须完成 WIZ90001 高级魔法课程。但是，本主题有许多先决条件，在学习WIZ90001之前必须完成每一个先决条件。这些学科中的每一门都可能有自己的先决条件，以此类推。</p>
</blockquote>
<p>Given a particular list of prerequisite requirements, the function <code v-pre>wiz_study_length(prereq_list, final='WIZ90001')</code> is intended to calculate the minimum number of semesters it will take Sandy to graduate. The arguments are:</p>
<blockquote>
<p>给定一个特定的先决条件要求列表，函数 <code v-pre>wiz_study_length(prereq_list, final='WIZ90001')</code> 用于计算 Sandy 毕业所需的最小学期数。的参数是:</p>
</blockquote>
<ul>
<li><code v-pre>prereq_list</code> , a list of <code v-pre>(subject, prerequisite)</code> tuples specifying that prerequisite must be completed before     enrolling in subject. Note that a subject may have more than one     prerequisite.</li>
</ul>
<blockquote>
<p>Prereq_list，一个(subject，prerequisite)元组列表，指定在注册 subject 之前必须完成先决条件。注意，一个主题可能有多个先决条件。</p>
</blockquote>
<ul>
<li>final, the target subject to     complete, with a default value of 'WIZ90001'.</li>
</ul>
<blockquote>
<p>final 是要完成的目标主题，默认值为'WIZ90001'。</p>
</blockquote>
<p>Any subject that appears as a prerequisite in a prereq_list tuple but does not appear as a subject can be assumed to have no prerequisites. Furthermore, we assume that Sandy has capacity to take as many subjects as required each semester, as long as they have completed their prerequisites.</p>
<blockquote>
<p>在 prereq_list 元组中作为先决条件出现但不作为主题出现的任何主题都可以假定为没有先决条件。此外，我们假设Sandy有能力每学期选修要求的课程，只要他们完成了他们的先决条件。</p>
</blockquote>
<p>For example:</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>In this case, it will take 4 semesters for Sandy graduate, due to the prerequisite chain: WIZ10001 -&gt; WIZ20001 -&gt; WIZ20003 -&gt; WIZ90001.</p>
<blockquote>
<p>在这种情况下，Sandy毕业需要4个学期，因为先决条件链:WIZ10001 -&gt;WIZ20001→WIZ20003→WIZ90001。</p>
</blockquote>
<div class="language-python line-numbers-mode" data-ext="py"><pre v-pre class="language-python"><code>•	<span class="token keyword">from</span> collections <span class="token keyword">import</span> defaultdict
...
<p>A school of minnows swims in the sea. Sharks eat minnows, according to the following strict rules. Sharks swim, one at a time, from left to right and eat each minnow they encounter, starting at the height of the first minnow that remains uneaten. Each time a shark eats a minnow it gets heavier and sinks one metre lower in the water. Once a shark reaches the sea floor, it is no longer able to eat minnows.</p>
<blockquote>
<p>一群小鱼在海里游泳。鲨鱼吃鲦鱼，要遵守以下严格的规则。鲨鱼一次游一条，从左到右，吃掉它们遇到的每一条鲦鱼，从第一条没有被吃掉的鱼的高度开始。鲨鱼每吃一条鲦鱼就会变重，沉入水下一米。一旦鲨鱼到达海底，它就不能再吃鲦鱼了。</p>
</blockquote>
<p>For example, minnows are located at the following heights (in metres) above the sea floor (e.g. a height of 3 indicates that the minnow is 3m above the sea floor) in sequence, left to right:</p>
<blockquote>
<p>例如，米诺鱼位于以下高度(以米为单位)(例如，高度为3表示米诺鱼位于海平面以上3米)，从左到右依次为:</p>
</blockquote>
<p><code v-pre>[4, 3, 1, 2, 4]</code></p>
<p>The first shark will start at height 4 and eat the minnow at that height (element 0), then sink to height 3 where it will eat the second minnow (element 1), and sink to height 2. It will pass above the third minnow (because it is at a lower height), before eating the fourth minnow (element 3) at height 2, and passing under the final minnow (element 4). At this point, the following minnows will remain:</p>
<blockquote>
<p>第一条鲨鱼从高度4开始，吃掉这个高度的米诺鱼(元素0)，然后沉到高度3，再吃第二条米诺鱼(元素1)，沉到高度2。它会从第三条米诺鱼的上方经过(因为它的高度较低)，然后在高度为2的地方吃掉第4条米诺鱼(元素3)，并从最后一条米诺鱼(元素4)的下方经过。此时，以下几条米诺鱼将继续存在:</p>
</blockquote>
<p><code v-pre>[None, None, 1, None, 4]</code></p>
<p>The second shark will start at height 1 and eat the minnow at that height (element 2), then sink to the sea floor (height 0) and eat no further minnows. This will result in the following minnows:</p>
<blockquote>
<p>第二条鲨鱼从1的高度开始吃米诺鱼(元素2)，然后沉到海底(高度0)，不再吃米诺鱼。这将导致以下小鱼:</p>
</blockquote>
<p><code v-pre>[None, None, None, None, 4]</code></p>
<p>The third shark will start at height 4 and eat the minnow at that height (element 4), after which no minnows will remain.</p>
<blockquote>
<p>第三条鲨鱼会从高度4开始，吃掉这个高度的米诺鱼(元素4)，之后就没有米诺鱼了。</p>
</blockquote>
<p>The function sharks_minnows(minnows, sharks) is intended to determine whether a given number of sharks will be sufficient to eat all of the minnows in the sea, positioned based on minnows. The function takes the following parameters:</p>
<blockquote>
<p>函数sharks_minnows(minnows，鲨鱼)的目的是确定给定数量的鲨鱼是否足以吃掉海里所有的minnows，根据minnows定位。该函数接受以下参数:</p>
</blockquote>
<ul>
<li>minnows, a list of positive integers indicating the height of each minnow, as in the example above.</li>
</ul>
<blockquote>
<p>Minnows，一个正整数列表，表示每条鲦鱼的高度，如上例所示。</p>
</blockquote>
<ul>
<li>sharks, a positive integer number of sharks.</li>
</ul>
<blockquote>
<p>鲨鱼的数量是正整数。</p>
</blockquote>
<p>The function should return True if the specified number of sharks is sufficient to eat all the minnows, and False otherwise.</p>
<blockquote>
<p>如果指定的鲨鱼数量足够吃掉所有的鲦鱼，函数应该返回True，否则返回False。</p>
</blockquote>
<div class="language-python line-numbers-mode" data-ext="py"><pre v-pre class="language-python"><code><span class="token keyword">def</span> <span class="token function">sharks_minnows</span><span class="token punctuation">(</span>minnows<span class="token punctuation">,</span> sharks<span class="token punctuation">)</span><span class="token punctuation">:</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>The provided code is imperfect, in that it sometimes returns <code v-pre>True</code> when it should return <code v-pre>False</code>, and sometimes returns <code v-pre>False</code> when it should return <code v-pre>True</code>.</p>
<blockquote>
<p>所提供的代码是不完美的，因为它有时在应该返回' False '时返回' True '，有时在应该返回' True '时返回' False '。</p>
</blockquote>
<p>(a) Provide an example of a function call where the provided code will correctly return <code v-pre>True</code> (i.e. a True Positive)</p>
<blockquote>
<p>提供一个函数调用的例子，其中所提供的代码将正确返回' True '(即True Positive)</p>
</blockquote>
<p>(b) Provide an example of a function call where the provided code will correctly return <code v-pre>False</code> (i.e. a True Negative)</p>
<blockquote>
<p>提供一个函数调用的例子，其中所提供的代码将正确返回' False '(即True Negative)</p>
</blockquote>
<p>(c) Provide an example of a function call where the provided code will <em>incorrectly</em> return <code v-pre>True</code> (i.e. a False Positive)</p>
<blockquote>
<p>提供一个函数调用的例子，其中提供的代码将<em>错误地</em>返回' True '(即假阳性)</p>
</blockquote>
<p>(d) Provide an example of a function call where the provided code will <em>incorrectly</em> return <code v-pre>False</code> (i.e. a False Negative)</p>
<blockquote>
<p>提供一个函数调用的例子，其中提供的代码将<em>错误地</em>返回' False '(即假阴性)</p>
</blockquote>
...
<p><strong>(a) Output value: 'eeezy'</strong></p>
<p>Required operations:</p>
<ol>
<li>
<p>string indexing</p>
...
<p><strong>(c) Output value: '100% pythonic'</strong></p>
<p>Required operations:</p>
<ol>
<li>
<p><code v-pre>f-string</code></p>
</li>
<li>
<p><code v-pre>/</code></p>
</li>
</ol>
<p><strong>(d) Output value: 'comp10001'</strong></p>
<p>Required operations:</p>
<ol>
<li>
<p>tuple indexing</p>
...
<li>
<p><code v-pre>.lower()</code></p>
</li>
</ol>
<p><strong>(e) Output value: '42'</strong></p>
<p>Required operations:</p>
<ol>
<li>
<p>list indexing</p>
...
<p>The following function is meant to take a lowercase string as input and return a reordered version of that string in which all of the vowels appear first (in the order in which they appear in the original string), followed by all of the consonants (again, in the order in which they appear in the original string), discarding any non-alphabetic characters in the process.</p>
<blockquote>
<p>下面的函数将接受一个小写字符串作为输入，并返回该字符串的重新排序版本，其中所有元音都在前面出现(按照它们在原始字符串中出现的顺序)，然后是所有辅音(同样，按照它们在原始字符串中出现的顺序)，在此过程中丢弃所有非字母字符。</p>
</blockquote>
<p>For example:</p>
<div class="language-python line-numbers-mode" data-ext="py"><pre v-pre class="language-python"><code><span class="token operator">>></span><span class="token operator">></span> reorder<span class="token punctuation">(</span><span class="token string">'stay at home'</span><span class="token punctuation">)</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>However, there are several errors in the given function definition.</p>
<blockquote>
<p>然而，在给定的函数定义中有几个错误。</p>
</blockquote>
<p>Identify <strong>exactly</strong> three (3) errors and specify: (a) the line number where the error occurs; (b) the type of error, as <em>syntax</em>, <em>run-time</em>, or logic; and (c) how you would fix each error, in the form of the corrected (single) line of code.</p>
<blockquote>
<p>准确识别 三(3)个错误，并指定:(a)错误发生的行号;(b)错误类型，如<em>语法</em>、<em>运行时</em>或逻辑;和(c)你将如何修复每个错误，以纠正的(单行)代码的形式。</p>
</blockquote>
<div class="language-python line-numbers-mode" data-ext="py"><pre v-pre class="language-python"><code><span class="token keyword">def</span> <span class="token function">reorder</span><span class="token punctuation">(</span>my_string<span class="token punctuation">)</span><span class="token punctuation">:</span>               <span class="token comment">#  1</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>找出三个错误并说出是那种错误（as syntax, run-time, or logic）并且改正</p>
